---
title: Tables
description: Tables
category: components
toc: true
---
import {Raw, Snippet} from "/src/components/snippet"

## Overview

Due to the widespread use of `<table>` elements across third-party widgets like calendars and date pickers, xiigrid's tables are **opt-in**. Add the base class `.table` to any `<table>`, then extend with our optional modifier classes or custom styles. All table styles are not inherited in xiigrid, meaning any nested tables can be styled independent from the parent.

Using the most basic table markup, here's how `.table`-based tables look in xiigrid.

<Snippet title={'Table 1'} preview={true}>
  <Fragment slot="html">
    <Raw file={file} line_start={19} line_end={55}>
      ```
      <table class="table">
        <thead class="top">
        <tr>
          <th scope="col">#</th>
          <th scope="col">First</th>
          <th scope="col">Last</th>
          <th scope="col">Twitter</th>
        </tr>
        </thead>
        <tbody class="middle">
        <tr>
          <td scope="row">1</td>
          <td>T</td>
          <td>B</td>
          <td>@tbd</td>
        </tr>
        <tr>
          <td scope="row">2</td>
          <td>A</td>
          <td>B</td>
          <td>@abc</td>
        </tr>
        <tr>
          <td scope="row">3</td>
          <td colspan="2">Void</td>
          <td>@nulllogic</td>
        </tr>
        </tbody>
        <tfoot class="bottom">
        <tr>
          <td>1</td>
          <td>2</td>
          <td>3</td>
          <td>4</td>
        </tr>
        </tfoot>
      </table>
      ```
    </Raw>
  </Fragment>
  <Fragment slot="scss">
    <Raw file={file} line_start={62} line_end={71}>
      ```
      @use '~/styles/app.scss' as app;

      $table: app.get-theme(app.$theme, 'components.table');

      @include app.generate-component(
      $table,
      'table',
      app.$config,
      app.$theme
      );
      ```
    </Raw>
  </Fragment>
</Snippet>

## Table striped

<Snippet title={'Table 2'} preview={true}>
  <Fragment slot="html">
    <Raw file={file} line_start={83} line_end={119}>
      ```
      <table class="table stripe">
        <thead class="top">
        <tr>
          <th scope="col">#</th>
          <th scope="col">First</th>
          <th scope="col">Last</th>
          <th scope="col">Twitter</th>
        </tr>
        </thead>
        <tbody class="middle">
        <tr>
          <td scope="row">1</td>
          <td>T</td>
          <td>B</td>
          <td>@tbd</td>
        </tr>
        <tr>
          <td scope="row">2</td>
          <td>A</td>
          <td>B</td>
          <td>@abc</td>
        </tr>
        <tr>
          <td scope="row">3</td>
          <td colspan="2">Void</td>
          <td>@nulllogic</td>
        </tr>
        </tbody>
        <tfoot class="bottom">
        <tr>
          <td>1</td>
          <td>2</td>
          <td>3</td>
          <td>4</td>
        </tr>
        </tfoot>
      </table>
      ```
    </Raw>
  </Fragment>
  <Fragment slot="scss">
    <Raw file={file} line_start={126} line_end={135}>
      ```
      @use '~/styles/app.scss' as app;

      $table: app.get-theme(app.$theme, 'components.table');

      @include app.generate-component(
        $table,
        'table',
        app.$config,
        app.$theme
      );
      ```
    </Raw>
  </Fragment>
</Snippet>
