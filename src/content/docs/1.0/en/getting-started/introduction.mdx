...
<Snippet file_path={file} clipboard={true} single={true} title={'🎐 Intro (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧'}>
  <Fragment slot={'code'}>
    <Raw file_path={file} lang={'scss'} highlights={['$accordion', 'p']} mark={['$accordion']}
         line_start={16}
         line_end={33}
      >
      @use 'sass:list';
      @use 'sass:string';
      @use 'sass:meta'; // [\!code error]
      @use 'sass:map'; // [\!code focus]
      ...
      $accordion: functions.get-theme(app.$theme, 'components.accordion'); // [\!code focus]
